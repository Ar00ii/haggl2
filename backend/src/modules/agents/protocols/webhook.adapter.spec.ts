...
describe('WebhookAdapter', () => {
  ...
  describe('validateConfig', () => {
    ...
    it('accepts a valid https URL', () => {
      expect(adapter.validateConfig({ endpoint: 'https://api.example.com/bolty' })).toBeNull();
    });
  });
  ...
  describe('healthCheck', () => {
    it('returns healthy on a 2xx response', async () => {
      ...
      const res = await adapter.healthCheck({ endpoint: 'https://api.example.com/bolty' });
    ...
    it('signs the outbound payload with HMAC when secret is set', async () => {
      ...
      await adapter.healthCheck({ endpoint: 'https://api.example.com/bolty' });
      expect(mockedAxios.post).toHaveBeenCalledTimes(1);
      const headers = mockedAxios.post.mock.calls[0][2]?.headers as Record<string, string>;
      expect(headers['X-Bolty-Event']).toBe('health_check');
    ...
    it('omits the signature header when secret is missing', async () => {
      ...
      await adapter.healthCheck({ endpoint: 'https://api.example.com/bolty' });
      const headers = mockedAxios.post.mock.calls[0][2]?.headers as Record<string, string>;
      expect(headers['x-bolty-signature']).toBeUndefined();
    });
  ...
  describe('invoke', () => {
    it('returns the parsed reply on a 200 with valid body', async () => {
      ...
      const res = await adapter.invoke(
        { endpoint: 'https://api.example.com/bolty' },
        { prompt: 'hi' },
      );
